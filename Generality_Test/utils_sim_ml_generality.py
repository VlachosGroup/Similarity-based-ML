...
def visualize_2Dscatter(reduced_data, labels, **kwargs):
    ...
    # max1 = np.max(reduced_data[:, 0])
    # max2 = np.max(reduced_data[:, 1])

    # maximum = np.min([max1, max2])

    # min1 = np.min(reduced_data[:, 0])
    # min2 = np.min(reduced_data[:, 1])
    
    # minimum = np.min([min1, min2])

        # Set the default font to Helvetica for all elements, including tick labels
    matplotlib.rcParams['font.family'] = 'Helvetica'

    # # Verify if Helvetica is not available, try Arial as an alternative
    # if 'Helvetica' not in plt.rcParams['font.family']:
    #     plt.rcParams['font.family'] = 'Arial'
    
    if len(list(set(labels))) <= 20:
...
# scatter = plt.scatter(reduced_data[:, 0], reduced_data[:, 1], c=data_cleaned['Viscosity (mPa*s)'], cmap='rainbow', vmin = 0, vmax = 5)
# cbar = plt.colorbar(scatter)
# cbar.set_label('Viscosity')
    # plt.title('Clustering of Molecules based on Structural Similarity')
    plt.show()
...
def getTopnFeatsRF_PI(X,y,n):
    rf_model1 = RandomForestRegressor(random_state=42)
    rf_model1.fit(X, y)

    # Compute permutation importance
    result = permutation_importance(rf_model1, X, y, scoring= 'neg_root_mean_squared_error',  n_repeats=30, random_state=42)
    ...
    # Get the sorted feature names and their importance scores
    sorted_features = [(X.columns[i], importances[i]) for i in sorted_indices1]
    ...
    result = pd.DataFrame({'Topfeats':topfeats, 'PI':importances})
    return result
...
def RF_custom(X, y, k, lower, upper, plot = True):
    # Assume X and y are your data and target values
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3)
